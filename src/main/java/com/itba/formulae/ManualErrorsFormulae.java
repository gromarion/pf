...
public class ManualErrorsFormulae {
	...
	public double computeIndividual(EvaluatedResource resource, int errorId, List<List<ResultItem>> properties)
			throws JSONException, IOException {
		Optional<EvaluatedResource> evaluatedResource = evaluatedResource(resource.getResource(),
	...
	public Score compute(String resource, Optional<EvaluationSession> session) throws JSONException, IOException {
		...
		List<List<ResultItem>> properties = new JsonSparqlResult(SparqlRequestHandler
				.requestResource(resource,
						session.isPresent() ? session.get().getCampaign()
		...
		double ans = 0;
		// XXX: si el documento aún no fue evaluado
	...
	private double erroredLinksOverTotalLinks(EvaluatedResource evaluatedResource, int errorId,
			List<List<ResultItem>> properties) {
		int numerator = 0;
		int denominator = 0;

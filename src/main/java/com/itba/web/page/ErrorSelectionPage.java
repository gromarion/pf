...
public class ErrorSelectionPage extends BasePage {
	...
	private final IModel<EvaluatedResource> evaluatedResource = new EntityModel<EvaluatedResource>(
			EvaluatedResource.class);
	...
	public ErrorSelectionPage(PageParameters parameters) {
		...
		if (parameters.getNamedKeys().contains("sessionId")) {
			evaluatedResource.setObject(evaluatedResourceRepo
					.getResourceForSession(evaluationSessionRepo.get(parameters.get("sessionId").toInt()), resource)
					.orNull());
		}
		...
		Form<ErrorSelectionPage> form = new Form<ErrorSelectionPage>("errorForm",
			...
			protected void onSubmit() {
				...
				Optional<EvaluatedResource> evaluatedResource = evaluatedResourceRepo
						.getResourceForSession(currentSession.getObject(), resource);
				...
				EvaluatedResourceDetail detail = new EvaluatedResourceDetail(evaluatedResource.get(),
						errorModel.getObject(), predicate, object);
				...
				try {
					Score score = manualErrorsFormulae.compute(resource,
							Optional.of(evaluatedResource.get().getSession()));
					evaluatedResource.get()
							.setScore(new BigDecimal(score.getScore()).setScale(3, BigDecimal.ROUND_HALF_EVEN));
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		boolean showForm = availableErrors.getObject().size() > 0
				&& (evaluatedResource.getObject() == null || isAuthor);
